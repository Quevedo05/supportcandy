...
interface SaveanFormularioBuilderProps {
...
function OpcionesEditor({ opciones, onUpdate }: OpcionesEditorProps) {
  ...
  return (
    <div>
      <label className="text-sm font-medium text-gray-700 mb-2 block">Opciones del selector</label>
  ...
  return (
    <>
      <div className="bg-white border border-gray-200 rounded-lg p-4 flex items-center gap-3 hover:shadow-sm transition">
        ...
        {campo.requerido && <span className="text-xs text-red-600 font-semibold">Requerido</span>}
      ...
      {isExpanded && (
        <div className="bg-slate-50 border border-gray-200 border-t-0 rounded-b-lg p-4 space-y-4 -mt-2 relative z-10">
          ...
          <div className="grid grid-cols-3 gap-4">
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-1">Etiqueta del campo</label>
              <input
                type="text"
                value={campo.label}
          ...
          {['texto', 'textarea', 'numero'].includes(campo.tipo) && (
            <div>
              <label className="text-sm font-medium text-gray-700 block mb-1">Texto de ayuda (placeholder)</label>
              <input
                type="text"
...
// Main component: SaveanFormularioBuilder
export function SaveanFormularioBuilder({ onGuardar, onCancelar }: SaveanFormularioBuilderProps) {
  ...
  useEffect(() => {
    const storedCampos = localStorage.getItem('sc_savean_formulario_campos');
    if (storedCampos) {
      try {
  ...
  const moverCampoArriba = (id: string) => {
    ...
    if (idx > 0) {
      ...
      [nuevosCampos[idx], nuevosCampos[idx - 1]] = [nuevosCampos[idx - 1], nuevosCampos[idx]];
  ...
  const actualizarCampo = (id: string, partial: Partial<CampoFormulario>) => {
    const nuevosCampos = campos.map((c) => (c.id === id ? { ...c, ...partial } : c));
    setCampos(nuevosCampos);
  };
  ...
  const handleGuardar = () => {
    ...
    localStorage.setItem('sc_savean_formulario_campos', JSON.stringify(campos));
    onGuardar(campos);
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-center gap-4 mb-6">
        ...
        <div className="flex-1">
          <h2 className="text-2xl font-bold text-gray-900">Configurar Formulario Público</h2>
        </div>
        <button
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        <p className="text-sm text-blue-700">
          <strong>ℹ️ Vista previa:</strong> El formulario que verá el camionero tendrá{' '}
